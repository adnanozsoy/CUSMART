 ...
 */
int sa_large(search_parameters params) { 
   ...
   /* Preprocessing */ 
   preSA(params.pattern, params.pattern_size, S); 
   F = 1 << (params.pattern_size - 1);

   /* Searching */ 
   for (D = 0, j = 0; j < params.text_size; ++j) { 
      D = ((D<<1)|1) & S[params.text[j]]; 
      if (D & F) {
         k = 0;
...
void sa(search_parameters params) { 
   unsigned int lim, D; 
   unsigned int S[SIGMA], F; 
   int j; 
   ...
   for (D = 0, j = 0; j < params.text_size; ++j) { 
      D = ((D<<1) | 1) & S[params.text[j]]; 
      if (D & F) params.match[j - params.pattern_size + 1] = 1; 
   } 
} 
