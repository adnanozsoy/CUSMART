...
void akc(search_parameters params) {
    ...
    /* Preprocessing */
    suff = (int *)malloc(params.pattern_size*sizeof(int));
    pref = (int *)malloc((params.pattern_size+1)*sizeof(int));
    clink = (int *)malloc(params.pattern_size*sizeof(int));
    slink = (int *)malloc(params.pattern_size*sizeof(int));
    skip = (int *)malloc(2*params.pattern_size*sizeof(int));
    bcGsVal = (Pair2 *)malloc(params.pattern_size*sizeof(Pair2));
    for (i=0; i < SIGMA; ++i) {
    ...
    preBmBc_(params.pattern, params.pattern_size, bmBc);
    suffixes_(params.pattern, params.pattern_size, suff);
    prePref(params.pattern_size, suff, pref);
    lsp = preLsp(params.pattern_size, suff);
    preBcGs_(params.pattern, params.pattern_size, suff, bcGsPtr, bcGsVal);
    preCLink(params.pattern, params.pattern_size, clink);
    preSLink(params.pattern, params.pattern_size, suff, pref, slink);

    /* Searching */
    j = 0;
    ell = 0;
    skip[0] = params.pattern_size;
    M=2*params.pattern_size;
    while (j <= params.text_size-params.pattern_size) {
        ...
        while (i >= 0) {
            ...
            if (params.pattern[i] == params.text[i+j]) {
                --i;
                --k;
            }
            else break;
        ...
        if (i < 0) {
            params.match[j] = 1;
            skip[ell] = params.pattern_size-lsp; 
            ell = (ell+1) % M;
            skip[ell] = lsp; 
        }
        else {
            skip[ell] = MAX(0,k-1); 
            ell = (ell+1) % M;
            skip[ell] = params.pattern_size-i; 
            ell = (ell+1) % M;
            if (i == params.pattern_size-1) {
                skip[ell] = bmBc[params.text[i+j]]-params.pattern_size+i+1; 
            }
            else {
                skip[ell] = getBcGS(params.text[i+j], i+1, bcGsPtr, bcGsVal); 
                if (skip[ell] == -1) {
                   skip[ell] = pref[i+1]; 
                }
            }
            validateShift(params.pattern_size, clink, slink, ell, skip);
        }
        j += skip[ell]; // shift
    }
    ...
    for (i=0; i < params.pattern_size; ++i) free(bcGsVal[i]);
    free(bcGsVal);
}
