#include "ebom_wrapper.h"
#include "algos/ebom.cuh"
#include "wrapper_helpers.h"
#include "util/cutil.cuh"
#include "util/tictoc.h"
#include "stddef.h"

search_info ebom_wrapper(search_parameters params){
	...
	int *h_S = (int *)malloc((params.pattern_size+1) * sizeof(int));
	...
	h_S[params.pattern_size] = params.pattern_size + 1; 
	for (i = params.pattern_size; i > 0; --i) { 
	  ...
	  h_S[iMinus1] = (p == params.pattern_size + 1 ? params.pattern_size : q); 
	} 
	...
	for (i=0; i<SIGMA; i++) { 
	  ...
	  for (j=0; j<SIGMA; j++) 
	    if (q>=0) h_FT[i][j] = h_trans[q][j]; 
	    else h_FT[i][j] = UNDEFINED; 
	}
