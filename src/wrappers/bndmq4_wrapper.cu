...
#include "bndmq4_wrapper.h"
#include "algos/bndmq4.cuh"
...
search_info backward_nondeterministic_dawg_qgram4_wrapper(search_parameters params){
	...
	if (params.pattern_size <=32) {
		backward_nondeterministic_dawg_qgram4<<<grid_dim, block_dim>>>(
			d_text, params.text_size, d_pattern, params.pattern_size, 
			d_B, M, params.stride_length, d_match);
	}
	else {
		backward_nondeterministic_dawg_qgram4_large<<<grid_dim, block_dim>>>(
			d_text, params.text_size, d_pattern, params.pattern_size, 
			d_B, M, params.stride_length, d_match);
	}
