...
#include "ww_wrapper.h"
#include "algos/ww.cuh"
#include "wrapper_helpers.h"
#include "util/cutil.cuh"
#include "util/tictoc.h"

search_info wide_window_wrapper(search_parameters params){
	...
	buildSimpleSuffixAutomaton(params.pattern, params.pattern_size, h_ttrans, h_tlength, h_tsuffix, h_tterminal);

	h_ttransSMA = (int *)malloc((params.pattern_size+1)*SIGMA*sizeof(int));
	memset(h_ttransSMA, -1, (params.pattern_size+1)*SIGMA*sizeof(int));
	preSMARev(params.pattern, params.pattern_size, h_ttransSMA);
	...
	wide_window<<<grid_dim, block_dim>>>(
