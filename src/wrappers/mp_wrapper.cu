...
#include "algos/mp.cuh"
#include "wrapper_helpers.h"
#include "util/cutil.cuh"
#include "util/tictoc.h"

search_info morris_pratt_wrapper(search_parameters params){
	...
	gpuErrchk( cudaMalloc((void**)&d_shift_array,	(params.pattern_size+1) * sizeof(int)) );

	// Setup: malloc > timer start > memset/memcpy
	wrapper_setup(params, &d_text, &d_pattern, &d_match);
	
	TicTocTimer preprocess_timer_start = tic();
	pre_morris_pratt(params.pattern, params.pattern_size, shift_array);
	...
	morris_pratt<<<grid_dim, block_dim>>>(
